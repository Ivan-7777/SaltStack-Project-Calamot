...
EOF
    ...
    chown root:root "/var/www/$nombre_dominio" || error "Fallo al cambiar la propiedad del directorio raíz."
    chmod 755 "/var/www/$nombre_dominio" || error "Fallo al establecer permisos en el directorio raíz."
    chown "$nombre_usuario":"$nombre_usuario" "/var/www/$nombre_dominio/pagina" || error "Fallo al cambiar la propiedad del directorio de la página."
    chmod 755 "/var/www/$nombre_dominio/pagina" || error "Fallo al establecer permisos en el directorio de la página."

    # Crear directorios necesarios para chroot SFTP
    info "Creando directorios necesarios para chroot SFTP..."
    mkdir -p "/var/www/$nombre_dominio/dev" || error "Fallo al crear directorio dev."
    mkdir -p "/var/www/$nombre_dominio/proc" || error "Fallo al crear directorio proc."

    # Crear dispositivos necesarios
    info "Creando dispositivos necesarios para chroot SFTP..."
    mknod "/var/www/$nombre_dominio/dev/null" c 1 3 2>/dev/null || true
    mknod "/var/www/$nombre_dominio/dev/zero" c 1 5 2>/dev/null || true
    mknod "/var/www/$nombre_dominio/dev/full" c 1 7 2>/dev/null || true
    mknod "/var/www/$nombre_dominio/dev/random" c 1 8 2>/dev/null || true
    mknod "/var/www/$nombre_dominio/dev/urandom" c 1 9 2>/dev/null || true
    mknod "/var/www/$nombre_dominio/dev/tty" c 5 0 2>/dev/null || true

    # Establecer permisos en dispositivos
    info "Estableciendo permisos en dispositivos..."
    chmod 666 "/var/www/$nombre_dominio/dev/null" 2>/dev/null || true
    chmod 666 "/var/www/$nombre_dominio/dev/zero" 2>/dev/null || true
    chmod 666 "/var/www/$nombre_dominio/dev/full" 2>/dev/null || true
    chmod 666 "/var/www/$nombre_dominio/dev/random" 2>/dev/null || true
    chmod 666 "/var/www/$nombre_dominio/dev/urandom" 2>/dev/null || true
    chmod 666 "/var/www/$nombre_dominio/dev/tty" 2>/dev/null || true
