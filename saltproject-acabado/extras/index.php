...
function minion_options($default) {
    $html = "";
    foreach (accepted_minions() as $minion) {
        $selected = $minion === $default ? " selected" : "";
        $safe = htmlspecialchars($minion, ENT_QUOTES, "UTF-8");
...
<div class="wrap">
    ...
    <form action="recibir.php" method="POST" autocomplete="off">
        ...
        <div class="panel state-panel" id="state-wireguard">
            <fieldset>
                ...
                <div class="grid">
                    ...
                    <div><label>AllowedIPs cliente</label><input type="text" name="wireguard[client_allowed_ips]" value="10.66.66.0/24,192.168.0.0/24"></div>
                    <div><label>Host WordPress cliente</label><input type="text" name="wireguard[client_web_host]" value="server.es"></div>
        ...
        <div class="panel" id="state-targets-panel">
            <fieldset>
                <legend>Destino de estados</legend>
                <p class="note">Solo aparecen los estados marcados. La lista sale automaticamente de los minions aceptados en el Salt master.</p>
                <div class="grid">
                    <div class="target-field" data-state-value="firewall"><label>Firewall</label><select name="targets[firewall]"><?php echo minion_options("minion-05"); ?></select></div>
                    <div class="target-field" data-state-value="dns"><label>DNS</label><select name="targets[dns]"><?php echo minion_options("minion-05"); ?></select></div>
                    <div class="target-field" data-state-value="proxy"><label>Proxy</label><select name="targets[proxy]"><?php echo minion_options("minion-04"); ?></select></div>
                    <div class="target-field" data-state-value="BDD"><label>BDD</label><select name="targets[BDD]"><?php echo minion_options("minion-02"); ?></select></div>
                    <div class="target-field" data-state-value="dhcp"><label>DHCP</label><select name="targets[dhcp]"><?php echo minion_options("minion-04"); ?></select></div>
                    <div class="target-field" data-state-value="pkica"><label>PKI CA</label><select name="targets[pkica]"><?php echo minion_options("minion-02"); ?></select></div>
                    <div class="target-field" data-state-value="wireguard"><label>WireGuard</label><select name="targets[wireguard]"><?php echo minion_options("minion-08"); ?></select></div>
                    <div class="target-field" data-state-value="webserver"><label>Webserver</label><select name="targets[webserver]"><?php echo minion_options("minion-02"); ?></select></div>
                    <div class="target-field" data-state-value="zabbix-server"><label>Zabbix server</label><select name="targets[zabbix-server]"><?php echo minion_options("minion-08"); ?></select></div>
                    <div class="target-field" data-state-value="restic-server"><label>Restic server</label><select name="targets[restic-server]"><?php echo minion_options("minion-08"); ?></select></div>
                </div>
            </fieldset>
        </div>
...
<script>
    ...
    function updateTargetSelectors() {
        ...
        document.querySelectorAll(".target-field").forEach((field) => {
            const state = field.dataset.stateValue;
            const checked = Array.from(document.querySelectorAll('input[name="states[]"][value="' + state + '"]')).some(cb => cb.checked);
        ...
        const panel = document.getElementById("state-targets-panel");
        if (panel) panel.style.display = visibleCount > 0 ? "block" : "none";
    }
