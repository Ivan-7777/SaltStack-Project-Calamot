...
dns:
  ...
  listen_on: []         # Las IPs se ponen en el Jinja del estado, usando otros pilares
  forwarders:
    - 8.8.8.8
    - 8.8.4.4
  zones:
    internal.local:
      type: master
      allow_transfer: none
    example.local:       # placeholder, se puede reemplazar en Jinja usando web-server.domain
      type: master
      allow_transfer: none
  hosts:
    ns1:
      ip: ""            # Se rellenará desde firewall.lan.ip en la plantilla Jinja
    www:
      ip: ""            # Se rellenará desde web-server.network.address
    mail:
      ip: ""            # Se rellenará desde web-server.network.address
    vpn:
      ip: ""            # Se rellenará desde wireguard.address
