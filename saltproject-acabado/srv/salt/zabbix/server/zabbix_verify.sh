...
if systemctl is-active --quiet apache2; then
    ...
    if [[ "$HTTP_CODE" == "200" || "$HTTP_CODE" == "302" ]]; then
        FRONTEND_BODY=$(curl -sf http://localhost/zabbix/ 2>/dev/null || true)
        if echo "$FRONTEND_BODY" | grep -qi "Configuration file error"; then
            log_fail "Frontend responde, pero muestra error de configuración"
        elif echo "$FRONTEND_BODY" | grep -Eqi "Sign in|Username|Zabbix"; then
            log_ok "Frontend Zabbix responde y muestra la pantalla de login (HTTP $HTTP_CODE)"
        else
